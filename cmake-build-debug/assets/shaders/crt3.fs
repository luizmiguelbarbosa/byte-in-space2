// crt.fs - Curvatura Forte e Pixels Máximos
#version 330
...
const float CURVATURE_AMOUNT = 0.35;      // Curvatura EXTREMA (efeito de "barriga")
...
const float VIGNETTE_FACTOR = 0.90;       // Vinheta Suave e Distante (quase sem escurecimento)
...
void main() {
    ...
    // Esta é a parte que tornará os "pixels" super visíveis.
    vec3 mask = vec3(0.0);
    float x = mod(uv_g.x * resolution.x, 3.0); // O padrão se repete a cada 3 pixels

    if (x < 1.0) mask = vec3(1.0, 0.0, 0.0); // Canal R
    else if (x < 2.0) mask = vec3(0.0, 1.0, 0.0); // Canal G
    else mask = vec3(0.0, 0.0, 1.0); // Canal B
    ...
    // --- 6. Vinhetas SUAVES (sem bordas pretas) ---
    // O fator VIGNETTE_FACTOR alto faz o escurecimento ser mínimo e longe do centro.
    vec2 pos_uv = uv - 0.5;
    float len = length(pos_uv);
    float vignette = smoothstep(VIGNETTE_FACTOR, VIGNETTE_FACTOR - 0.4, len); 
    color.rgb *= vignette;
    
    // --- 7. Garantir que as bordas da tela curva não cortem a imagem ---
    // Em vez de preto, usamos a vinheta suave para dar um 'fade' na imagem que saiu da curva.
    float border_fade = 1.0;
    if (uv_g.x < 0.0 || uv_g.x > 1.0 || uv_g.y < 0.0 || uv_g.y > 1.0) {
        border_fade = vignette;
    }
    color.rgb *= border_fade;
